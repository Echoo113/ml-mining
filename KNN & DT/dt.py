...
class Leaf(Node):
    ...
    def predict_class_probabilities(self, X: ArrayLike) -> np.ndarray:
        ...
        """
        # >>> YOUR CODE HERE >>>
        probabilities = np.array([self.class_probabilities[label] for label in self.class_labels]).reshape(1, -1)
        n = X.shape[0]
        result = np.tile(probabilities, (n, 1))
        # <<< END OF YOUR CODE <<<
    ...
    def predict(self, X: ArrayLike) -> np.ndarray:
        ...
        labels = self.class_labels[predicted_indices]
        # <<< END OF YOUR CODE <<<
        return labels
...
class DecisionTree:
    ...
    def _build_tree(self, X: ArrayLike, y: ArrayLike, max_depth: int, exclude: set = set()) -> Node:
        ...
        if 'NA' not in children:
            # >>> YOUR CODE HERE >>>
            classes=np.unique(y)
            default_prob = {label: 1.0 / len(classes) for label in classes}
            children['NA'] = Leaf(default_prob)
            # <<< END OF YOUR CODE <<<

        # Lastly, we create a split node, which splits on the best feature.
        # Return a split node.
        # >>> YOUR CODE HERE >>>
        split = Split(feature_of_split, children)
        return split
        # <<< END OF YOUR CODE <<<
