...
class NaiveBayes:
    ...
    def compute_class_probabilities(self, y_train: np.ndarray) -> Dict[Any, float]:
        ...
        class_probabilities = {
            int(c): float((count + self.alpha) / (total + self.alpha * n_classes))
            for c, count in zip(classes, counts)
        }
        #Expected:{1: 0.375, 2: 0.25, 3: 0.375}
    ...
    def compute_feature_probabilities(self, X_j_train: np.ndarray, y_train: np.ndarray) -> Dict[Tuple[Any, Any], float]:
        ...
        for c in class_labels:
            # >>> YOUR CODE HERE >>>
            indices = (y_train == c)
            X_j_class = X_j_train[indices]
            total_count = len(X_j_class)
            V = len(unique_features)  # vocabulary size
            for feature in unique_features:
                count_feature = np.sum(X_j_class == feature)
                # Apply Laplace smoothing correctly
                probability = (count_feature + self.alpha) / (total_count + self.alpha * V)
                feature_probabilities[(str(feature), int(c))] = float(probability)
            # <<< END OF YOUR CODE <<<
    ...
    def predict_probabilities(self, X_test: np.ndarray) -> np.ndarray:
        ...
        for i in range(X_test.shape[0]):
            ...
            for c, label in enumerate(self.class_labels):
                ...
                # >>> YOUR CODE HERE >>>
                probabilities[i, c] = np.log(self.class_probabilities[int(label)])
                # <<< END OF YOUR CODE >>>
                ...
                for j in range(self.n_features):
                    ...
                    # probability of NA
                    if (str(X_test[i, j]), label) not in self.feature_probabilities[j]:
                        # >>> YOUR CODE HERE >>>
                        probabilities[i, c] += np.log(self.feature_probabilities[j][('NA', int(label))])
                        # <<< END OF YOUR CODE >>>
                    else:
                        # If the feature value is in the training data, use the
                        # probability of the feature value
                        # >>> YOUR CODE HERE >>>
                        probabilities[i, c] += np.log(self.feature_probabilities[j][(str(X_test[i, j]), int(label))])
                        # <<< END OF YOUR CODE >>>
    ...
    def predict(self, probabilities: np.ndarray) -> np.ndarray:
        ...
        y_pred = self.class_labels[indices]
        y_pred = y_pred.ravel()
        # <<< END OF YOUR CODE <<<
        return y_pred
    ...
    def evaluate(self, y_test: np.ndarray, probabilities: np.ndarray) -> Tuple[float, float]:
        ...
        # >>> YOUR CODE HERE >>>
        m = len(y_test)
        zero_one_loss = np.sum(y_pred.ravel() != y_test.ravel()) / m
        squared_errors = []
        for i in range(m):
            # Find the index of the true label within self.class_labels
            idx = np.where(self.class_labels == y_test[i])[0][0]
            p_true = probabilities[i, idx]
            squared_errors.append((1 - p_true) ** 2)
        squared_loss = np.mean(squared_errors)
