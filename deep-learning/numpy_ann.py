...
class NumpyANN():
    ...
    def _backward(self, X: np.ndarray, y_true: np.ndarray) -> None:
        ...
        # >>> YOUR CODE HERE >>>
        dz1 = np.dot(dz2, self.W2.T) * self._relu_grad(self.z1)
        # <<< END OF YOUR CODE <<<
        ...
        # >>> YOUR CODE HERE >>>
        dW1 = np.dot(X.T, dz1) / m                      # ∂Loss/∂W1
        db1 = np.sum(dz1, axis=0, keepdims=True) / m    # ∂Loss/∂b1
        # <<< END OF YOUR CODE <<<
...
if __name__ == "__main__":
    ...
    if doctest.testmod(optionflags=doctest.ELLIPSIS).failed == 0:
        ...
        ann = NumpyANN(input_size=784, hidden_size=512, output_size=10, lr=0.01)
        ann.train(X_train, y_train, X_val, y_val, epochs=250, batch_size=16)
